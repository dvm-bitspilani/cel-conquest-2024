...
export default function Home() {
  ...
  function handleClick() {
    ...
    try {
      ...
      const startTime = `${timestamp.toDate(dataRef.current.slot_start_time)}`.split(' ')[4].slice(0, -3)
      const endTime = `${timestamp.toDate(dataRef.current.slot_end_time)}`.split(' ')[4].slice(0, -3)

      let date
      if (`${timestamp.toDate(dataRef.current.slot_start_time)}`.split(' ')[2][0] === '1' && `${timestamp.toDate(dataRef.current.slot_start_time)}`.split(' ')[2].length === 2) {
        date = `${timestamp.toDate(dataRef.current.slot_start_time)}`.split(' ')[2] + 'th';
      }
      else {
        switch (`${timestamp.toDate(dataRef.current.slot_start_time)}`.split(' ')[2][1]) {
          case '1':
            date = `${timestamp.toDate(dataRef.current.slot_start_time)}`.split(' ')[2] + 'st';
            break;
          case '2':
            date = `${timestamp.toDate(dataRef.current.slot_start_time)}`.split(' ')[2] + 'nd';
            break;
          case '3':
            date = `${timestamp.toDate(dataRef.current.slot_start_time)}`.split(' ')[2] + 'rd';
            break;
          default:
            date = `${timestamp.toDate(dataRef.current.slot_start_time)}`.split(' ')[2] + 'th';
            break;
      ...
      let month
      switch (`${timestamp.toDate(dataRef.current.slot_start_time)}`.split(' ')[1]) {
        case 'Jan':
          month = 'January'
          break;
        case 'Feb':
        ...
        case 'Dec':
          ...
          break;
        default:
          month = `${timestamp.toDate(dataRef.current.slot_start_time)}`.split(' ')[1]
          break;
      }
